...
const buildDir = await mkdtemp(join(tmpdir(), 'maden-debug-markdown-'));
const outfile = join(buildDir, 'debug-markdown-entry.mjs');
...
await build({
  ...
  bundle: true,
  entryPoints: [join(repoRoot, 'scripts/debug-markdown-entry.ts')],
  format: 'esm',
  jsx: 'automatic',
  loader: {
    '.css': 'empty',
