...
function Draggable(props: PlateElementProps) {
  ...
  return (
    ...
    >
      ...
      <div
        ref={previewRef}
        className={cn('slate-drag-preview -left-0 absolute hidden w-full')}
...
const DragHandleButton = React.memo(
  React.forwardRef<HTMLButtonElement, {
    ...
    { dragButtonTop, isDragging, previewRef, resetPreview, setPreviewTop },
    ref
  ) {
    const editor = useEditorRef();
    const element = useElement();
...
const DropLine = React.memo(function DropLine({
  ...
  return (
    <div
      ...
      className={cn(
        'slate-dropLine',
        'absolute inset-x-0 z-40 h-0.5 opacity-100 transition-opacity',
        'bg-brand/50',
        dropLine === 'top' && '-top-px',
