...
const EquationPopoverContent = ({
  ...
  return (
    <PopoverContent
      className={cn(
        isInline
          ? 'flex !flex-row min-w-[320px] items-center gap-2 p-2'
          : 'flex gap-2 p-2.5'
      )}
      onEscapeKeyDown={(e) => {
        e.preventDefault();
    ...
    >
      <EquationInput
        className={cn(
          isInline
            ? 'h-8 max-h-8 grow resize-none rounded-md border border-transparent bg-transparent px-2 py-1 text-sm leading-6 shadow-none outline-none focus:border-border/60 focus:outline-none focus-visible:ring-0'
            : 'max-h-[50vh] grow resize-none rounded-md border border-input/40 bg-input/30 p-2 text-sm shadow-none outline-none focus:outline-none focus-visible:ring-0',
          className
        )}
        minRows={isInline ? 1 : 2}
        maxRows={isInline ? 1 : 12}
      ...
      <Button
        ...
        className={cn(isInline ? 'h-8 shrink-0 px-3' : 'px-3')}
        onClick={onClose}
      >
